const REGISTRATION = 'REGISTRATION',
  LOGIN = 'LOGIN',
  IS_ME = 'IS_ME',
  LOGOUT = 'LOGOUT',
  IS_NOT_ME = 'IS_NOT_ME',
  ORG_ADD_ORG = 'ORG_ADD_ORG',
  ORG_KEY_IN_DEP = 'ORG_KEY_IN_DEP',
  DEP_ADD_DEP = 'DEP_ADD_DEP',
  DEP_TO_ORG = 'DEP_TO_ORG',
  DEP_ARR_AT_DEP = 'DEP_ARR_AT_DEP',
  DEP_ACTUAL = 'DEP_ACTUAL',
  WORKER_TO_DEP = 'WORKER_TO_DEP',
  MAIN_IS_CREATOR = 'MAIN_IS_CREATOR',
  MAIN_IS_NOT_CREATOR = 'MAIN_IS_NOT_CREATOR',
  MAIN_USER = 'MAIN_USER',
  MAIN_DEPARTMENTS = 'MAIN_DEPARTMENTS',
  MAIN_ORGANIZATIONS = 'MAIN_ORGANIZATIONS',
  MAIN_CREATOR_ORGANIZATIONS = 'MAIN_CREATOR_ORGANIZATIONS',
  MAIN_CREATOR_DEPARTMENTS = 'MAIN_CREATOR_DEPARTMENTS',
  DEP_CLEAR_ACTUAL = 'DEP_CLEAR_ACTUAL',
  BACK_WORKER_DEPS = 'BACK_WORKER_DEPS',
  IS_LOADING = 'IS_LOADING',
  NOT_LOADING = 'NOT_LOADING';

export {
  REGISTRATION,
  LOGIN,
  IS_ME,
  LOGOUT,
  IS_NOT_ME,
  ORG_ADD_ORG,
  DEP_ADD_DEP,
  DEP_TO_ORG,
  ORG_KEY_IN_DEP,
  DEP_ARR_AT_DEP,
  DEP_ACTUAL,
  WORKER_TO_DEP,
  MAIN_IS_CREATOR,
  MAIN_IS_NOT_CREATOR,
  MAIN_USER,
  MAIN_DEPARTMENTS,
  MAIN_ORGANIZATIONS,
  MAIN_CREATOR_ORGANIZATIONS,
  MAIN_CREATOR_DEPARTMENTS,
  DEP_CLEAR_ACTUAL,
  BACK_WORKER_DEPS,
  IS_LOADING,
  NOT_LOADING
}
